...
const serviceAccount = require("./lunettes-1e40f-firebase-adminsdk.json");
      ...
      res.json(result)
        ...
        app.delete("/purchase/:id", async (req, res) => {
         ...
         app.put("/users/admin",verifyConnection,  async (req, res) => {
          ...
          // if(requester) {
          //   const reqsterAccount = userCollection.findOne({email: requester});
          ...
          const requester = req.decodedEmail;
          if(requester){
            const reqsterAccount = await userCollection.findOne({email: requester});
            if(reqsterAccount.role === "admin"){
              const filter =  { email: user.email}
          const updateDoc = {$set:{role: "admin"}};
          const result = await userCollection.updateOne(filter,updateDoc)
